...
if(has_capability('mod/contextadmin:editowncatsettings', $context)) {
    ...
    $table->define_headers(array($stractivitymodule, "$strhide/$strshow", $stroverride_heading, $strlocked_heading, $strclear_heading, $strsettings));
}
...
foreach ($modules as $module) {
    ...
    if (file_exists("$CFG->dirroot/local/contextadmin/mod/$module->name/cat_settings.php") && has_capability('mod/contextadmin:editowncatsettings', $context)) {
        $settings_td = "<a href=\"cat_settings.php?section=modsetting$module->name&name=$module->name&contextid=$contextid\">$strsettings</a>";
    } else {
        $settings_td = "";
    }
    ...
    if(has_capability('mod/contextadmin:changevisibilty', $context)) {
        ...
        if(category_module_exists($catid,$module->name)){
            $clear_td = create_form($OUTPUT,$module->name."_clear_form",$self_path,$strhide,'cross_red_big',array('module_name'=>$module->name, 'clear'=>'true', 'sesskey'=>sesskey()));

            if ($module->override) {
                $override_td = create_form($OUTPUT,$module->name."_override_form",$self_path,$strhide,'completion-manual-y',array('module_name'=>$module->name, 'override'=>'false', 'sesskey'=>sesskey()));
                $class   = '';
            } else {
                $override_td = create_form($OUTPUT,$module->name."_override_form",$self_path,$strhide,'completion-manual-n',array('module_name'=>$module->name, 'override'=>'true', 'sesskey'=>sesskey()));
            }

            if ($module->locked) {
                $locked_td = create_form($OUTPUT,$module->name."_locked_form",$self_path,$strhide,'completion-manual-y',array('module_name'=>$module->name, 'locked'=>'false', 'sesskey'=>sesskey()));
                $class   = '';
            } else {
                $locked_td = create_form($OUTPUT,$module->name."_locked_form",$self_path,$strhide,'completion-manual-n',array('module_name'=>$module->name, 'locked'=>'true', 'sesskey'=>sesskey()));
            }
